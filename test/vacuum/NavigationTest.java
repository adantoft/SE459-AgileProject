...
public class NavigationTest {
	...
	public void runVacuumSmallSpaceTest() throws Exception {
		...
		for (Tile tile : map.getTiles()) { // tests that all tiles are not
											// visited
	...
	public void runVacuumExampleFloorTest() throws Exception {
		...
		for (Tile tile : map.getActiveTiles()) { // tests that all tiles are not
													// visited
			assertEquals(tile.getVisited(), 0);
		}
	...
	public void shortestDistanceSingleRoomTest() throws Exception {
		...
		System.err.println("End tile coordinates: X: " + endTileX + " Y: "
				+ endTileY);

		FloorPlan map = new FloorPlan(xSize, ySize);
		Space testRoomBare = new Space(new Point(lowerLeft, lowerLeft),
				new Point(upperRight, upperRight));
		map.setSpace(0, testRoomBare, BARE);
		...
		assertNotEquals(cs.getTile().getAdjacentTiles().size(), 0);
		ArrayList<Tile.Direction> successPath = Navigation.calculatePath(
				cs.getTile(), endTile);
		...
		for (Tile.Direction dir : Tile.Direction.values()) {
			System.err.print(dir + ": "
					+ Collections.frequency(successPath, dir) + " ");
		}
		System.err.println();
	...
	public void cleanSweepReturnLowPowerTest() throws Exception {
		...
		// System.err.println("End Tile is coordinates: X: " + endTileX + " Y: "
		// +endTileY);
