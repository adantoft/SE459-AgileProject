...
public class CleanSweepTest {
	...
	/*
	 * @Test public void cleanTest() throws Exception { Tile tile1 = new Tile(0,
	 * BARE); Tile tile2 = new Tile(32, BARE); Tile tile3 = new Tile(32, BARE);
	 * tile1.attachTile(tile2, NORTH); tile2.attachTile(tile3, EAST);
	 * cs.setTile(tile1);
	 * 
	 * cs.move(NORTH); assertFalse(cs.getTile().hasDirt());
	 * assertTrue(cs.getDirtBag() == 32);
	 * 
	 * cs.move(EAST); assertTrue(cs.getTile().hasDirt());
	...
	public void obstacleTest() throws Exception {
		...
		assertNull(cs.getTile().getAdjacent(SOUTH)); // Detecting a nearby
														// wall/edge
	...
	public void moveBackTest() throws Exception {
		...
		assertFalse(cs.moveBack()); // CS should remain on same tile since it
									// cannot move back
		assertNotEquals(t2, cs.getTile());
		assertEquals(t1, cs.getTile());
		...
		// the below is testing if the visit history stack is properly
		// implemented
		assertFalse(cs.moveBack()); // CS should remain on same tile since it
									// cannot move back
